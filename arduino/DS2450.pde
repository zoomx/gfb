...
void ReadDS2450(uint8_t* deviceAddress)
{
...
//ignores CRC16 as we can just check the readback bits
void Configure_2450(uint8_t* deviceAddress, int vrange, int rez)
{  
  if(!parasite) {             // Not needed if using parasite power
    ds.reset();
  ...
  ds.write(rez,0);            // 0008 (Channel A Control/Status) - rez bits
  ds.read();                  //Read back 16 bit CRC
  ds.read();                  //"
  if(ds.read() != rez) {      //check readback byte
    Serial.println("Readback verification failed, attempting to continue");
  }
   
  ds.write(vrange, 0);        // 0009 (Channel A Control/Status) - VDC range
  ds.read();                  //Read back 16 bit CRC
  ds.read();                  //"
  if(ds.read() != vrange) {   //check readback byte
    Serial.println("Readback verification failed, attempting to continue");
  }
  
  ds.write(rez, 0);           // 000A (Channel B Control/Status) - rez bits
  ds.read();                  //Read back 16 bit CRC
  ds.read();                  //"
  if(ds.read() != rez) {      //check readback byte
    Serial.println("Readback verification failed, attempting to continue");
  ...
  ds.write(rez, 0);           // 000C (Channel C Control/Status) - rez bits
  ds.read();                  //Read back 16 bit CRC
  ds.read();                  //"
  if(ds.read() != rez) {      //check readback byte
    Serial.println("Readback verification failed, attempting to continue");
  ...
  ds.write(rez, 0);           // 000E (Channel D Control/Status) - rez bits
  ds.read();                  //Read back 16 bit CRC
  ds.read();                  //"
  if(ds.read() != rez) {      //check readback byte
    Serial.println("Readback verification failed, attempting to continue");
